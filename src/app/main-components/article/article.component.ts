...
export class ArticleComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if(this.postId){
      console.log('lll');
      var url=`${'http://localhost:8000/api/1.0/article/'+this.postId}`;
  ...
  onSaveArticle(){
    ...
    if(this.articleForm.value.idpost){
      var url=`${'http://localhost:8000/api/1.0/article/'+this.articleForm.value.idpost}`;
      ...
      var url='http://localhost:8000/api/1.0/article/new/';
